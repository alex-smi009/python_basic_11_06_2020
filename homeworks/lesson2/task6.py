...
feature_db = ['название товара', 'цена', 'количество', 'ед']
number = 1
dbase = [] #base
analitics = {}

while True:
    data_in = input(f'Введите данные товара № {number} через пробел {feature_db} или "exit" для окончания\n')
    if data_in == 'exit':
        break
    else:
        tup = (number, dict(zip(feature_db, data_in.split(' '))))
        dbase.append(tup)
        number += 1
